...
According to the recommendation from experts, we need to use provinces instead of ecoregions. We can use the table from the spatial object to make the link.
...
mgv.meow %<>% left_join(provs,by="ECO_CODE")
```
...
```{r}
mgv.meow %>% mutate(area=st_area(geometry),centroid=st_centroid(geometry)) %>% st_drop_geometry  %>% transmute(oid,ECO_CODE,PROV_CODE,area,lon=st_coordinates(centroid)[,1],lat=st_coordinates(centroid)[,2]) -> rslts_meow
...
For the terrestrial ecoregions we bind together the objects in the Rdata files:
...
Now we can summarise by ecoregion id or province id:
```{r}
...
rslts_meow %>% group_by(PROV_CODE) %>% summarise(total_area=sum(area) %>% set_units("km²"), n_polygons=n_distinct(oid), x=weighted.mean(lon,weight=area), y=weighted.mean(lat,weight=area)) -> rsm_meow
...
Now we can map by terrestrial ecoregions... (however the terrestrial ecoregions were not recommended for this functional group, we keep this code for reference)
...
```{r}
world.plot + labs(title=sprintf("Mangrove polygons (from GMW 2016) aggregated by Marine Provinces: %s distinct units", nrow(rsm_meow)),caption="Circles indicate centroid of group",color = "Nr. of polygons",size="Area [km^2]") +
  geom_point(data=rsm_meow,aes(x=x,y=y,size=total_area %>% drop_units,color=n_polygons))-> p_provinces
p_provinces
...
world.plot + labs(title=sprintf("Mangrove polygons (from GMW 2016) aggregated by Large Marine Ecosystems: %s distinct units", nrow(rsm_lmes)),caption="Circles indicate centroid of group",color = "Nr. of polygons",size="Area [km^2]") +
  geom_point(data=rsm_lmes,aes(x=x,y=y,size=total_area %>% drop_units,color=n_polygons)) -> p_lmes
p_lmes
```
...
ggsave(plot=p_lmes,file=sprintf('%s/Mangrove-aggregated-by-LME.pdf',work.dir),device=pdf)
ggsave(plot=p_provinces,file=sprintf('%s/Mangrove-aggregated-by-MEOW-provinces.pdf',work.dir),device=pdf)
...
```{r}
mgv %>% mutate(area=st_area(geom),centroid=st_centroid(geom)) %>% st_drop_geometry  %>% transmute(oid=ogc_fid,area,lon=st_coordinates(centroid)[,1],lat=st_coordinates(centroid)[,2])  -> mgv_units

rslts_teow %>% filter(!is.na(area)) %>% transmute(oid=ogc_fid,TEOW_ID=sprintf("T%s",ECO_ID)) %>% full_join( rslts_meow %>% transmute(oid,MEOW_ID=sprintf("M%s",PROV_CODE)) , by="oid") %>% full_join( rslts_lmes %>% transmute(oid,LME_ID=sprintf("L%s",LME_NUMBER)) , by="oid") %>% full_join(mgv_units,by="oid") -> mgv_xtab

mgv_xtab %>% summarise(total=n(),units=n_distinct(oid))

mgv_xtab %>% group_by(TEOW_ID,MEOW_ID,LME_ID) %>% summarise(n_poly=n_distinct(oid),total_area=sum(area) %>% set_units("km^2"), x=weighted.mean(lon,weight=area), y=weighted.mean(lat,weight=area))
...
mgv_xtab %>% filter(!is.na(MEOW_ID)) %>% group_by(node=MEOW_ID) %>% summarise(n_poly=n_distinct(oid),total_area=sum(area) %>% set_units("km^2"), x=weighted.mean(lon,weight=area), y=weighted.mean(lat,weight=area)) %>%
   bind_rows (mgv_xtab %>% filter(!is.na(TEOW_ID)) %>% group_by(node=TEOW_ID) %>% summarise(n_poly=n_distinct(oid),total_area=sum(area) %>% set_units("km^2"), x=weighted.mean(lon,weight=area), y=weighted.mean(lat,weight=area))) %>%
   bind_rows(mgv_xtab %>% filter(!is.na(LME_ID)) %>% group_by(node=LME_ID) %>% summarise(n_poly=n_distinct(oid),total_area=sum(area) %>% set_units("km^2"), x=weighted.mean(lon,weight=area), y=weighted.mean(lat,weight=area))) %>% mutate(group=substr(node,1,1)) -> nodes
...
Still work to do: use centroids to display the nodes/links on a map

```{r}
l <- as.matrix(data.frame(V(g)$x,V(g)$y))

##plot(g, layout=l,vertex.label.cex=.5)
#require(maps)
#plot('world')
#map('world', fill = TRUE, col = "aliceblue")
#plot(g, layout=l,vertex.label.cex=.5,add=T,rescale=F)

unzip(sprintf("%s/admin/global/World-Bank/wb_land_10m.zip",gis.data))
world.sf <- read_sf("WB_Land_10m/WB_Land_10m.shp")

E(g)$width <- sqrt(E(g)$total_area/100)

V(g)$size <- sqrt(V(g)$total_area)*10
V(g)$label <- ifelse(V(g)$size > 3,V(g)$name,NA)

par(mar=rep(0,4))
plot(st_geometry(world.sf) %>% st_crop(xmin=-180,ymin=-40,xmax=180,ymax=40))
plot(g, layout=l,vertex.label.cex=.5,add=T,rescale=F)
