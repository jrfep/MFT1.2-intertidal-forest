...
if (file.exists(key_spp_file)) {
  ...
  write_csv(key_spp_data,file=key_spp_file)
}
```
...
mprov.xy %>% left_join(post_units_components,by="ECO_CODE") %>% st_drop_geometry() %>% select(ECO_CODE,unit_code,unit_name) -> a
...
a %>% bind_cols(b) %>% 
  ...
  mutate(Species=gsub("_"," ",Species),unit_name=gsub("Mangroves of ","",unit_name)) %>% 
  mutate(unit_name=gsub("outh|orth|erate|ical|est|ast|ern","",unit_name)) %>% 
...
mga_spp %>%  inner_join(spp_mtz, by=c("scientific_name"="Species")) -> mga_spp_data
  ...
  write_csv(mga_spp_data,path=mga_spp_file)
}
