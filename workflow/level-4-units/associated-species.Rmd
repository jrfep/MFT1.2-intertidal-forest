    ...
    for (spp in unique(names(gbif.data$gbif$data))) {
      if (!(spp %in% colnames(sppXprov)) & !(spp %in% colnames(sppXcell)) & (nrow(gbif.data$gbif$data[[spp]])>0)) {
        ...
        if (nrow(xys)>0) {
          ...
          save(file=mi.rda,sppXcell,sppXprov,mangrove_species)
        }
      }
    }
  }
...
for (spp in list_spps) {
  ...
  save(file=mi.rda,sppXcell,sppXprov,grd2Xcell,grd2Xprov,mangrove_species)
...
save(file=mi.rda,mtzXcell,mtzXprov,mangrove_species)
