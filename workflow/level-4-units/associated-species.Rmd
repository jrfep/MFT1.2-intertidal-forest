...
mangroves %>% st_drop_geometry %>% group_by(kingdom,phylum,class,order_) %>% summarise(nspp=n_distinct(id_no))
 ...
 meow %>% filter(ECO_CODE %in% rownames(mgv.spp)) %>% group_by(PROV_CODE) %>% summarise %>% st_geometry %>% plot
...
d1 <- vegdist(mgv.spp,'kulczynski',binary=T)
...
save(file=mi.rda, mangroves, mgv.spp)
