...
mi.rda <- sprintf("%s/apps/L4map/assmntdata.rda",script.dir)
...
if (file.exists(mi.rda)) {
  load(mi.rda)
  faltan <- faltan[!faltan %in% rslts_appox$PROV_CODE]
} else {
...
for (j in faltan) {
  ...
  rslts %>% filter(year %in% 2016) %>% transmute(PROV_CODE,Class,Sedimentar,Area_2016=area_ogr,EOO,AOO) -> a1
  rslts %>% filter(year %in% 1996) %>% transmute(PROV_CODE,Class,Sedimentar,Area_1996=area_ogr) -> a2
  
  rslts_approx %<>% bind_rows(
    a1 %>% left_join(a2) %>% 
      transmute(PROV_CODE, Class, Sedimentar, Area=Area_2016, 
                Area_Change=((Area_2016-Area_1996)/Area_1996) %>% set_units('%'),
                AOO,EOO)) 
...
# rslts %>% transmute(overest=area_poly/area_ogr) %>% summarise(range(overest))

save(file=mi.rda,all_rslts,rslts_approx)
