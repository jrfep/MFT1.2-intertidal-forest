...
AT.id <- newXMLNode("AT-id",sprintf("MFT1.2-RES-%02d",cdg))
AT.descriptions <- newXMLNode("AT-descriptions",
  children=list(newXMLNode("AT-description", attributes=list(lang="en"),
  "[INSTRUCTIONS: Add here a brief description of the ecosystem type must provide contextual information on its classification; clearly describe four elements that define the ecosystem type (characteristic native biota; abiotic environment; key processes and interactions; and spatial distribution); and describe the threats and collapsed states.]")))
AT.names <- newXMLNode("AT-names",
    children=list(newXMLNode("AT-name", attributes=list(lang="en"), sprintf("Intertidal forests and shrublands of %s", unit_name)),
    newXMLNode("AT-name", attributes=list(lang="es"), sprintf("Bosques de manglar de %s", unit_name))))
...
newXMLNode("Classification-element", sprintf("MFT1.2 RES %03d %s",cdg,unit_name), attrs=list(level="4"), parent=class.typ)
...
output.file <- sprintf("%s/xml/Assessment_target_MFT1.2-RES-%02d.xml",script.dir,cdg)

saveXML(doc,file=output.file,
     prefix = '<?xml version="1.0" encoding="UTF-8"?>
     <?xml-stylesheet type="text/css" href="target.css"?>',
     encoding = "UTF-8")
