...
provs <- prov_summary  %>% group_by(PROVINCE,PROV_CODE) %>%
  ...
  PROV_CODE %in% c(6,9,11,47,53,54,16,28,44) ~ "Potential merge",
...
prov.mgv.spp %>% st_drop_geometry %>% group_by(PROV_CODE,PROVINCE) %>% summarise(across(Dolichandrone.spathacea:Ceriops.australis, max)) -> mtz
 ...
 DT::renderDataTable({
   mtz %>% rowwise() %>% transmute(PROV_CODE,PROVINCE,sum=sum(c_across(Dolichandrone.spathacea:Ceriops.australis))) %>% full_join(prov_summary,by=c('PROV_CODE','PROVINCE'))  %>% transmute(PROVINCE,`Area 2000`=area_2020 %>% set_units('km^2') %>% round(2),`Area 2016`=radius %>% set_units('m^2') %>% set_units('km^2') %>%  round(2),`Mangrove species`=sum) %>% full_join(st_drop_geometry(filteredData()),by=c('PROV_CODE','PROVINCE'))-> dts
